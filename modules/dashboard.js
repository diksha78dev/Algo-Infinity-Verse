...
function initDashboard() { updateDashboard(); if (typeof updateProfile === 'function') updateProfile(); }
...
function updateActivityList() {
  ...
  const completedProblems = Array.isArray(userProgress.completedProblems) ? userProgress.completedProblems : [];
  if (completedProblems.length === 0) { activityList.innerHTML = '<p class="empty-state">No recent activity. Start solving problems!</p>'; return; }
  const activities = completedProblems.slice(-5).map(pid => { const problem = findProblemById(practiceProblems, pid); return { problem: problem ? problem.title : "Unknown", time: "Today" }; });
...
function updateRecentProblems() {
  ...
  if (recentProblems.length === 0) { container.innerHTML = '<p class="empty-state">No recently viewed problems</p>'; return; }
  const recentItems = recentProblems.map(id => findProblemById(practiceProblems, id)).filter(Boolean);
  if (recentItems.length === 0) { container.innerHTML = '<p class="empty-state">No recently viewed problems</p>'; return; }
  container.innerHTML = recentItems.map(problem => `<button class="recent-problem" type="button" data-id="${problem.id}">${escapeHtml(problem.title)}</button>`).join("");
  container.querySelectorAll(".recent-problem").forEach(item => { item.addEventListener("click", () => { const problem = findProblemById(practiceProblems, item.dataset.id); if (problem && typeof openQuizEditor === 'function') openQuizEditor(problem); }); });
}
...
function updateBadges() {
  ...
  const completedProblems = Array.isArray(userProgress.completedProblems) ? userProgress.completedProblems : [];
  const badges = [{ id: 1, icon: "🌟", name: "First Steps", description: "Begin your journey", criteria: "Solve 1 problem", earned: completedProblems.length >= 1 }, { id: 2, icon: "🔥", name: "On Fire", description: "Keep the momentum going", criteria: "Maintain a 7-day streak", earned: (userProgress.streak || 0) >= 7 }, { id: 3, icon: "💎", name: "Diamond", description: "Reach a major XP milestone", criteria: "Earn 5,000 XP", earned: (userProgress.xp || 0) >= 5000 }, { id: 4, icon: "🚀", name: "Rocket", description: "Speed through problems", criteria: "Solve 50 problems", earned: completedProblems.length >= 50 }, { id: 5, icon: "👑", name: "Master", description: "Achieve expert problem-solving", criteria: "Solve 100 problems", earned: completedProblems.length >= 100 }, { id: 6, icon: "🎯", name: "Sharpshooter", description: "Hit the target with consistency", criteria: "Solve 25 problems and earn 2,500 XP", earned: completedProblems.length >= 25 && (userProgress.xp || 0) >= 2500 }];
  const earned = badges.filter(b => b.earned).map(b => b.id);
  if (JSON.stringify(earned) !== JSON.stringify(userProgress.badges)) { userProgress.badges = earned; if (typeof saveUserData === 'function') saveUserData(); }
...
async function loadLeaderboard() {
  ...
  if (!apiAbort || !apiCache) return { leaders: [], currentUserId: null };
...
function getCurrentLeaderboardEntry(currentUserId = getCurrentUserId()) {
  const userProgress = window.userProgress || {};
  return normalizeLeaderboardEntry({ id: currentUserId || "local-user", name: getCurrentDisplayName(), xp: userProgress.xp, level: userProgress.level, avatar: userProgress.avatar });
}

function getCurrentUserId() { return window.algoAuth?.user?.sub || window.algoAuth?.user?.id || (typeof cachedSession !== 'undefined' ? cachedSession?.user?.sub : null) || "local-user"; }

function getCurrentDisplayName() { return window.algoAuth?.user?.name || (typeof cachedSession !== 'undefined' ? cachedSession?.user?.name : null) || (window.userProgress ? window.userProgress.name : null) || "Learner"; }
...
function renderLeaderboardRows(rows, currentUserId = getCurrentUserId(), options = {}) {
  ...
  leaderboardList.innerHTML = rows.map(user => {
    ...
    return `<div class="leaderboard-item ${isCurrentUser ? 'current-user' : ''}"><span class="leader-rank">#${user.rank}</span><span class="leader-avatar" aria-hidden="true">${avatarHtml}</span><span class="leader-name">${escapeHtml(displayName)}</span><span class="leader-xp">${user.xp.toLocaleString()} XP</span></div>`;
  }).join("");
}
...
function renderActivityHeatmap() {
  ...
  // Format date helper inside here
...
export { initDashboard, updateDashboard, updateLeaderboard, updateBadges, updateFreezeHistoryList, renderActivityHeatmap };
